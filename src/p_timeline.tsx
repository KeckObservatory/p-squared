...
import { ControlState, Employee, EntryState, DATE_FORMAT, HAWAII_TIMEZONE, DATETIME_FORMAT } from './control'
...
export const PTimeline = (props: Props) => {
    ...
    const lct = new Date()
    const tzoffset = lct.getTimezoneOffset() * 60 * 60 * 1000; //offset in milliseconds
    ...
    const make_groups_and_items = (entries: EntryData[],
        ...
        for (let idx = 0; idx < newItems.length; idx++) { //ensure idx are all unique
            ...
            if (state.unit.includes('month')) { //month items are a full day
                ...
                newItems[idx].start_time = newItems[idx].start_time.startOf('day')
                newItems[idx].end_time = newItems[idx].end_time.endOf('day')
    ...
    const onItemClick = (itemId: number, evt: any, time: any) => {
        ...
        console.log('matches_name', matches_name, 'name', props.name, 'group', item.group, 'canEdit', props.canEdit)
        if (matches_name || props.canEdit) {
            ...
            props.setEntryState((entryState: EntryState) => {
                return ({
                    ...
                    department: item.department ?? employee?.BaseCamp ?? entryState.department,
                    baseCamp: item.baseCamp ?? employee?.BaseCamp ?? entryState.baseCamp,
                    startHour: st.hour(),
                    endHour: et.hour(),
                    startMinutes: st.minute(),
                    endMinutes: et.minute(),
                    dateRange: [item.start_time.toDate(), item.end_time.toDate()],
