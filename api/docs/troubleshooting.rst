...
G. **LOAR entries are not showing up**
   ...
   Check cron scripts as `kcron@vm-hqcronserver`. There should be a row that runs the script `/home/kcron/bin/crons/LOAR/LOARtoPP.py` Try running them manually and see what the error is by debugging script.
